    ...
    _createClass(Element, [
      {
        ...
        value: function routeEvents(el, block, options) {
          ...
          // for devices that support Force Touch
            ...
            this.adapter = new AdapterForceTouch(
              el,
              block,
              options
            ).bindEvents();
    ...
    _createClass(Adapter, [
      ...
      },
      {
        ...
        value: function runPolyfill(event) {
          this.increment =
            Config.get("polyfillSpeedUp", this.options) === 0
              ? 1
              : 10 / Config.get("polyfillSpeedUp", this.options);
          this.decrement =
            Config.get("polyfillSpeedDown", this.options) === 0
              ? 1
              : 10 / Config.get("polyfillSpeedDown", this.options);
          this.setPressed(true);
          this.runClosure("start", event);
          if (this.runningPolyfill === false) {
  ...
  var AdapterForceTouch = (function (_Adapter) {
    ...
    function AdapterForceTouch(el, block, options) {
      ...
      return _possibleConstructorReturn(
        this,
        (
          AdapterForceTouch.__proto__ ||
          Object.getPrototypeOf(AdapterForceTouch)
        ).call(this, el, block, options)
      );
    }
  ...
  var Adapter3DTouch = (function (_Adapter2) {
    ...
    function Adapter3DTouch(el, block, options) {
      ...
      return _possibleConstructorReturn(
        this,
        (
          Adapter3DTouch.__proto__ || Object.getPrototypeOf(Adapter3DTouch)
        ).call(this, el, block, options)
      );
    }
      ...
      },
      {
        ...
        value: function supportLegacy(iter, event, runKey, force) {
          if (force !== this.initialForce) {
            ...
            iter++;
            setTimeout(
              this.supportLegacy.bind(this, iter, event, runKey, force),
              10
            );
          } else {
            this.fail(event, runKey);
        ...
        value: function selectTouch(event) {
          if (event.touches.length === 1) {
            ...
            for (var i = 0; i < event.touches.length; i++) {
              ...
              if (
                event.touches[i].target === this.el ||
                this.el.contains(event.touches[i].target)
              ) {
  ...
  var AdapterPointer = (function (_Adapter3) {
    ...
    function AdapterPointer(el, block, options) {
      ...
      return _possibleConstructorReturn(
        this,
        (
          AdapterPointer.__proto__ || Object.getPrototypeOf(AdapterPointer)
        ).call(this, el, block, options)
      );
    }
